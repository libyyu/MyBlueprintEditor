...
// ============================================================================
// Emscripten / WebGL 文件对话框
// 打开：通过 JS 调用 <input type="file"> 弹出系统文件选择器，结果写入虚拟 FS
// 保存：直接操作 Emscripten 虚拟文件系统（MEMFS），并触发浏览器下载
// ============================================================================
#include <emscripten.h>
#include <emscripten/val.h>
#include <unistd.h>

// 同步等待 JS 文件选择结果（通过共享全局变量）
// 由于浏览器文件选择是异步的，此处实现为：
//   1. 弹出 JS 对话框
//   2. 旋转等待（emscripten_sleep）直到 JS 回调写入结果
// 注意：需要链接 -sASYNCIFY 或使用 Asyncify，否则只能返回空串（非阻塞）
std::string OpenFileDialog(const char* /*filter*/, const char* /*title*/)
{
    // 通过 EM_ASM 弹出原生文件选择器，结果存入全局 JS 变量
    // 使用 emscripten_browser_file 或简单 input element
    EM_ASM({
        var input = document.createElement('input');
        input.type = 'file';
        input.accept = '.bjson,.json';
        input.onchange = function(e) {
            var file = e.target.files[0];
            if (!file) { Module._bp_file_result = ""; return; }
            var reader = new FileReader();
            reader.onload = function(ev) {
                var data = new Uint8Array(ev.target.result);
                var path = '/tmp/' + file.name;
                FS.writeFile(path, data);
                Module._bp_open_path = path;
                Module._bp_open_done = 1;
            };
            reader.readAsArrayBuffer(file);
        };
        Module._bp_open_done = 0;
        Module._bp_open_path = "";
        input.click();
    });

    // 非阻塞方式：等待最多 30 秒（需要 ASYNCIFY）
    // 若不支持 Asyncify 则直接返回空（调用方重试）
    int waited = 0;
    while (!EM_ASM_INT({ return Module._bp_open_done || 0; }) && waited < 30000) {
        emscripten_sleep(16);
        waited += 16;
    }
    ...
    char pathBuf[512] = {};
    EM_ASM({
        var p = Module._bp_open_path || "";
        stringToUTF8(p, $0, 512);
    }, pathBuf);
...
std::string SaveFileDialog(const char* /*filter*/, const char* /*title*/, const char* defaultExt)
{
    // Web 端保存：返回一个虚拟路径，调用方写入后再触发浏览器下载
    std::string ext = defaultExt ? defaultExt : "bjson";
    return std::string("/tmp/blueprint_save.") + ext;
}

// Web 端保存后触发浏览器下载（在 DoSaveFile 写完文件后调用）
void TriggerBrowserDownload(const std::string& virtualPath, const std::string& filename)
{
    EM_ASM({
    ...
    }, virtualPath.c_str(), filename.c_str());
}
...
void BlueprintEditor::LoadEditorData(const RTBlueprintData& data)
{
    ...
    for (const auto& rtNode : data.nodes)
    {
        ...
        if (def && !skipReconcile)
        {
            ...
            // 使用 EditorTypes.h 中的共享工具函数 ParseEnumValues
            for (size_t i = 0; i < def->inputPins.size(); ++i)
            {
