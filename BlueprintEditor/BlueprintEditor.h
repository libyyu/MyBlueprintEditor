...
public:
    ...
    // Comment 节点内联编辑状态（同样用于 CustomEventNode/FireEvent 节点标题重命名）
    ed::NodeId     editingCommentId   = 0;         // 当前正在编辑标题的节点 ID
...
struct BlueprintEditor : public Application
{
    ...
    int         m_PendingSwitchTabIndex = -1;  // 待切换 Tab（context + 导航）
    int         m_PendingTabVisualIndex = -1;  // 待切换 Tab 视觉（SetSelected，下一帧 TabBar 消费）
    ...
    struct ClipboardNode
    {
        ...
        struct ClipboardPin
        {
            ...
            std::vector<Pin::EnumEntry> enumValues;  // 枚举候选项（复制时一并保存）
