...
//-----------------------------------------------------------------------------
// Custom texture management functions (project-specific)
//-----------------------------------------------------------------------------
...
static TEXTURE* ImGui_FindTextureByView(ImTextureID textureID)
{
    ID3D11ShaderResourceView* view = (ID3D11ShaderResourceView*)(intptr_t)textureID;
    for (int i = 0; i < g_Textures.Size; ++i)
        if (g_Textures[i]->View == view)
            return g_Textures[i];
    return nullptr;
}
...
ImTextureID ImGui_CreateTexture(const void* data, int width, int height)
{
    ...
    // Return the SRV pointer as ImTextureID so that the DX11 render loop can use it directly
    return (ImTextureID)(intptr_t)texture->View;
}

void ImGui_DestroyTexture(ImTextureID textureID)
{
    if (textureID == ImTextureID_Invalid)
        return;

    TEXTURE* texture_object = ImGui_FindTextureByView(textureID);
    if (!texture_object)
        return;
...
int ImGui_GetTextureWidth(ImTextureID textureID)
{
    if (TEXTURE* tex = ImGui_FindTextureByView(textureID))
        return tex->Width;
    return 0;
}

int ImGui_GetTextureHeight(ImTextureID textureID)
{
    if (TEXTURE* tex = ImGui_FindTextureByView(textureID))
        return tex->Height;
    return 0;
