...
Blueprint.RegisterHandler("OnBeginPlay", function(ctx)
    -- 从蓝图变量读取配置
    local api_key    = ctx:GetVariable("ApiKey")   or "sk-your-api-key-here"
    local base_url   = ctx:GetVariable("BaseURL")  or "https://api.openai.com/v1"
    local model      = ctx:GetVariable("Model")    or "gpt-4o"
    local user_query = ctx:GetVariable("UserQuery")
        or "What's the weather in Beijing and Shanghai today, and any recent AI news?"
