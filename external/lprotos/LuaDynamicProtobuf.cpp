...
static int push_stack(lua_State* l, int64_t value)
{
	...
	lua::push(l, (lua::int64)value);
	return 1;
}
static int push_stack(lua_State* l, uint64_t value)
{
	//lua_pushlstring(l, (const char*)&value, 8);
	lua::push(l, (lua::uint64)value);
	return 1;
}

static pb::uint64 checkUInt64(lua_State* L, int n)
{
	/*if (lua_isnumber(L, n) && lua_type(L, n) == LUA_TNUMBER)
	{
		lua_Number nr = luaL_checknumber(L, n);
		return (pb::int64)nr;
	}
	size_t len;
...
public:
	...
	void BindRef(lua_State * L)
	{
		...
		lua::add_lua_close_cleaner(L, [](lua_State* l)->int {
			MyReaderBuffer::get().UnBindRef(gL);
			return 0;
		});
	}
	void UnBindRef(lua_State * L)
	{
		if (readFunc != LUA_NOREF && L)
		{
			luaL_unref(L, LUA_REGISTRYINDEX, readFunc);
			readFunc = LUA_NOREF;
		}
	}
};
...
public:
	...
	{
		//std::cout << "Error: " << fileName.c_str() << "@" << line << ":" << column << ", err:" << message.c_str() << std::endl;
		lua::lua_printerror(gL, "LoadFileDescriptor Occured Error %s, %s:%d:%d", message.c_str(), fileName.c_str(), line, column);
	}
public:
...
/*
	...
	成功返回true,失败返回
*/
static bool _LoadFileDescriptor(char const* pFileName, const bool bSetParse, std::vector<const pb::FileDescriptor*>& fd, std::string& err)
{
	for (auto& p : fd)
	{
		if (p->name() == pFileName)
	...
	const pb::FileDescriptor* pFindFileDescriptor = descriptorPool().FindFileByName(pFileName);
	if (pFindFileDescriptor)
	{
		fd.push_back(pFindFileDescriptor);
		lua::lua_log(gL, "FileDescriptor %s already in pool", pFileName);
		return true;
	}

	lua::lua_log(gL, "Load FileDescriptor %s", pFileName);
	size_t size = 0;
	...
	if (bSetParse)
	{
		...
		if (!fileDescriptorSet.ParseFromArray(pBuffer, size)) 
		{
			bSucceed = false;
			err = std::string("Parse ") + pFileName;
		}

		file_descriptor_proto = fileDescriptorSet.file(0);
	}
	else
	{
	...
	
	{
		...
		for (int i = 0; i < dependency_count_; ++i)
		{
			...
			if (!_LoadFileDescriptor(dependency_proto.c_str(), bSetParse, fd, err))
			{
				return false;
			}
		}
...
static int LoadFileDescriptor(lua_State *L)
{
	...
	bool bSetParse = luaL_optnumber(L, 2, 0) != 0;

	std::string err;
	std::vector<const pb::FileDescriptor*> fileDescriptors;
	bool succ = _LoadFileDescriptor(pFileName, bSetParse, fileDescriptors, err);
	if (!succ){
...
extern "C"
{
	LUALIB_API int luaopen_lprotos(lua_State *L)
	{
		gL = L;
		{
			...
			luaL_newlib(L, FileDescriptor_funcs);
			lua_rawset(L, -3);

			lua_pushstring(L, "Descriptor");
			luaL_newlib(L, Descriptor_funcs);
			lua_rawset(L, -3);

			lua_pushstring(L, "EnumDescriptor");
			luaL_newlib(L, EnumDescriptor_funcs);
			lua_rawset(L, -3);

			lua_pushstring(L, "EnumValueDescriptor");
			luaL_newlib(L, EnumValueDescriptor_funcs);
			lua_rawset(L, -3);

			lua_pushstring(L, "FieldDescriptor");
			luaL_newlib(L, FieldDescriptor_funcs);
			lua_rawset(L, -3);

			lua_pushstring(L, "UnknownField");
			luaL_newlib(L, UnknownField_funcs);
			lua_rawset(L, -3);

			lua_pushstring(L, "Message");
			luaL_newlib(L, Message_funcs);
			lua_rawset(L, -3);

			lua_pushstring(L, "ExtensionRangeOptions");
			luaL_newlib(L, ExtensionRangeOptions_funcs);
			lua_rawset(L, -3);
			top = lua_gettop(L);
			lua_pushvalue(L, -1);
			luaL_setfuncs(L, DynamicProtobuf_funcs, 0);
			top = lua_gettop(L);
		}
		return 1;
	}
	LUALIB_API int luaopen_liblprotos(lua_State* L)
	{
		return luaopen_lprotos(L);
	}
	LUALIB_API void unload_lprotos()
	{
		MyReaderBuffer::get().UnBindRef(gL);
		gL = NULL;
	}
	LUALIB_API void unload_liblprotos()
	{
		return unload_lprotos();
	}
};
