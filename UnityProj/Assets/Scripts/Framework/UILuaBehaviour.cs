...
public class UILuaBehaviour : MonoBehaviour, IUIPanelBridge {
    ...
    private GameObject FindByName(string name)
    {
        ...
        if (tr != null) { _qCache[name] = tr.gameObject; return tr.gameObject; }
    ...
    public void SetText(string name, string text)
    {
        ...
        if (go == null) { Debug.LogWarning($"[UILuaBehaviour] SetText: '{name}' not found"); return; }
        ...
        if (btnTmp != null) { btnTmp.text = text; return; }
        var btnTxt = go.GetComponentInChildren<UnityEngine.UI.Text>(true);
        if (btnTxt != null) { btnTxt.text = text; return; }
    ...
    public void RegisterClick(string name, LuaFunction callback)
    {
        ...
        var go = FindByName(name);
        if (go == null) { Debug.LogWarning($"[UILuaBehaviour] RegisterClick: '{name}' not found"); return; }
        var btn = go.GetComponent<Button>();
        if (btn == null) { Debug.LogWarning($"[UILuaBehaviour] RegisterClick: '{name}' has no Button"); return; }
