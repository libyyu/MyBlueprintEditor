...
// Compatibility: ImRect::Floor() was removed in newer ImGui versions
static inline void ImRect_Floor(ImRect& r) { r.Min.x = ImFloor(r.Min.x); r.Min.y = ImFloor(r.Min.y); r.Max.x = ImFloor(r.Max.x); r.Max.y = ImFloor(r.Max.y); }
